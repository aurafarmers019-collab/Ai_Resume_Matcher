...
DB_HOST      = "localhost"
DB_USER      = "root"
DB_PASSWORD  = "12345678"           # ← your MySQL root password
DB_NAME      = "resume_matcher"

SMTP_EMAIL    = "yourapp@gmail.com"     # ← Gmail address
...
def _extract_skills(text: str, threshold: float = 0.52) -> List[str]:
    ...
    for skill in SKILLS_LIST:
        if skill in text_lower:
            found.append(skill)
    # semantic fallback for skills not found exactly
...
def _skill_evidence(skill: str, sentences: List[str]) -> str:
    """Return the resume sentence with highest semantic similarity to the skill."""
    if not sentences:
        return ""
...
def _analyse(resume_text: str, jd_text: str) -> Dict[str, Any]:
    ...
    # batch encode resume + JD together
    ...
    if resume_years is not None and jd_years is not None:
        ...
        exp_match = 0.5  # neutral if not detectable
...
def send_otp(email: str = Form(...), purpose: str = Form(...), background_tasks: BackgroundTasks = BackgroundTasks()):
    # Check email exists in DB for reset purpose
    if purpose == "reset":
        db = get_db(); cur = db.cursor(dictionary=True)
    ...
    # Try sending email directly (not as background task to catch errors)
    try:
        _send_otp_email(email, otp, purpose)
    except Exception as e:
        print(f"⚠️  OTP email failed: {e}")
        # Don't fail the request — OTP is stored, user can retry
        # But log the actual OTP so dev can test without SMTP
...
def sync_password(email: str = Form(...), new_password: str = Form(...)):
    """Called after Firebase password reset to keep bcrypt hash in sync."""
    db = get_db(); cur = db.cursor(dictionary=True)
    cur.execute("SELECT * FROM users WHERE email=%s", (email,))
    user = cur.fetchone()
    if not user:
    ...
    cur.execute("UPDATE users SET password=%s WHERE email=%s", (pw_hash, email))
    db.commit()
    user["password"] = None  # don't return hash
    cur.close(); db.close()
...
async def upload_profile_resume(user_id: int, resume: UploadFile = File(...)):
    """Save or replace the candidate's profile resume."""
    if not resume.filename.lower().endswith(".pdf"):
        raise HTTPException(400, "Only PDF files are accepted.")
    contents = await resume.read()
    # Store in uploads folder with user-specific name
    import os
...
def download_profile_resume(user_id: int):
    """Download the candidate's saved resume."""
    import os
